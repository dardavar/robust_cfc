...
if __name__ == '__main__':
    ...
    num_of_errors_to_apply = all_bbs_df['size'].value_counts().apply(lambda x: int(NUM_OF_ERRS_PER_SIZE/x)).to_dict()
    all_bbs_df['num_of_errs_to_apply'] = all_bbs_df.apply(lambda row: num_of_errors_to_apply[row['size']], axis=1)
    all_bbs_df['rand_err_same_size_err_in_sig'] = \
        all_bbs_df.progress_apply(lambda row:
